...
@section('content')
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="d-flex my-auto btn-list justify-content-end">
                                    <!-- ROUTE UNTUK MENUJU FORM INPUT -->
                                    <a href="{{ route('data_pengguna.input') }}" class="btn btn-sm btn-primary"><i
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            ...
                                            <!-- BUTTON UNTUK MENJALANKAN EXPORT EXCEL -->
                                            <a class="dropdown-item" href="javascript:void(0)" onclick="exportPdf()">PDF</a>
                                            <!-- BUTTON UNTUK MENJALANKAN EXPORT PDF -->
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md-3">
                                ...
                                <!-- SELECT UNTUK MEM FILTER DATA -->
                                <select id="f1" class="form-control select2" onchange="reload_table()">
                                    <option value="">=== semua ===</option>
                                    <option value="1" @if (request()->get('f1') == 1) selected @endif>administrator
                                    </option>
                                    <option value="2" @if (request()->get('f1') == 2) selected @endif>operator
                                    </option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="table-responsive">
                            <table id="tbl_list" class="table table-sm table-striped table-bordered tx-14" width="100%">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($data as $dt)<!-- MEMBUAT PERULANGAN UNTUK MENAMPILKAN HASIL DARI DB -->
                                        <tr>
                                            <td>{{ $no++ }}</td>
                                            <td><img width="100px" height="60px" class="rounded-5"
                                                    src="@if ($dt->img) {{ asset('') }}images/user/{{ $dt->img }} @else {{ asset('') }}images/no-user.png @endif"
                                                    style="object-fit:cover"> </td>
                                            <td>{{ $dt->name ?? '' }}</td>
                                            <td>{{ $dt->email ?? '' }}</td>
                                            <td>{{ $dt->namerole ?? '' }}</td>
                                            <td>{{ isset($dt->created_at) ? $Tanggal->inddatetime($dt->created_at ?? '', ' ') : '' }}
                                            </td>
                                            <td>
                                                <form onsubmit="return confirm('Apakah Anda Yakin ?');"
                                                    action="{{ route('data_pengguna.destroy', $dt->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a href="{{ route('data_pengguna.edit', $dt->id) }}" title="Edit"
    ...
    <div class="modal fade effect-scale" id="mdl_formImport" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" id="area_formImport" role="document">
            <div class="modal-content modal-content-demo">
                <form id="formImport" action="{{ route('data_pengguna.import_excel') }}" method="post"
                    enctype="multipart/form-data">
                    @csrf
                    <div class="modal-header">
                        <h6 class="modal-title">Import Data</h6><button aria-label="Close" class="btn-close"
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="alert alert-info alert-dismissible fade show" role="alert">
                                    ...
                                    <span class="alert-inner--text"><strong>Catatan!</strong><br> sistem akan mereplace data
                                        jika menginputkan kode yang sama dengan data yang sudah tersimpan di
                                        aplikasi.</span>
                                ...
                                <div class="row row-xs align-items-center mg-b-20">
                                    ...
                                    <div class="col-md-8 mg-t-5 mg-md-t-0">
                                        <a href="{{ asset('') }}format_datapengguna.xlsx"
                                            class="btn btn-sm btn-secondary" download><i class="fa fa-download me-2"></i>
                                            Download Format Excel Import</a>
                                    </div>
                                </div>
                    ...
                    <div class="modal-footer">
                        <button type="submit" class="float-right btn btn-primary pd-x-30 mg-r-5 mg-t-5"><i
    ...
    <script>
        ...
        // FUNGSI UNTUK RELOAD TABLE KALO USER MEM FILTER DATA
        function reload_table() {
            var f1 = $('#f1').val();
            window.location.href = "data_pengguna?f1=" + f1;
        }

        // FUNGSI UNTUK MENAMPILKAN MODAL UNTUK IMPORT EXCEL
        function formImport() {
            $("#formImport")[0].reset();
        ...
        function exportExcel() {
            ...
            window.open(
                ...
                '_blank' // <- HALAMAN AKAN DI ARAHKAN SESUAI KONFIGURASI
            );
        }
        // FUNGSI UNTUK MENGAMBIL DATA
        function exportPdf() {
